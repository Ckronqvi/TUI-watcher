...
pub async fn get_tui_price(url: &str) -> Result<String, Box<dyn Error>> {
    ...
    let request = client.request(reqwest::Method::GET, url).header(USER_AGENT, "curl");
    let response = request.send().await?.text().await?;
    // Parse the HTML content
    let document = Html::parse_document(&response);
    // Parse all the scripts in the HTML content
    let selector = Selector::parse("script").map_err(|e| {
        io::Error::new(io::ErrorKind::Other, format!("Failed to parse selector: {}", e))
    })?;
    ...
    for script in scripts {
        ...
        if script_text.contains("currentPrice") {
            ...
            if
                let (Some(json_start), Some(json_end)) = (
                    script_text.find('{'),
                    script_text.rfind('}'),
                )
            {
                ...
                match serde_json::from_str::<Value>(json) {
                    Ok(value) => {
                        if let Some(product) = value.get("product") {
                            ...
                            return Err(
                                Box::new(
                                    io::Error::new(io::ErrorKind::Other, "product key not found")
                                )
                            );
                        }
                    }
    ...
    Err(Box::new(io::Error::new(io::ErrorKind::Other, "Script containing currentPrice not found")))
}
